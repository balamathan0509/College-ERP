...
export default function PrincipalQuestionPaperReview() {
  ...
  const fetchPendingPapers = async () => {
    ...
    try {
      const q = query(
        ...
        where("status", "==", "SUBMITTED_TO_PRINCIPAL")
      );
      const snap = await getDocs(q);
      const list = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      list.sort((a, b) => (b.submittedToPrincipalAt?.toMillis() || 0) - (a.submittedToPrincipalAt?.toMillis() || 0));
      setPapers(list);
    } catch (err) {
      console.error(err);
  ...
  const handleAction = async (decision) => {
    ...
    try {
      const newStatus = decision === 'APPROVED' ? 'APPROVED' : 'RETURNED_TO_HOD';
      const historyEntry = {
        action: decision === 'APPROVED' ? 'Principal Final Approved' : 'Principal Returned to HOD',
      ...
      const updateData = {
        status: newStatus,
        principalReviewedAt: new Date().toISOString(),
        principalReviewedBy: userProfile.uid,
      ...
      toast.success(`Paper successfully ${decision === 'APPROVED' ? 'final approved' : 'returned to HOD'}.`);
    ...
    <div className='dashboard-wrapper'>
      ...
      <main className='main-content'>
        ...
        <div className='page-header'>
          <h1>Principal Question Paper Final Approval</h1>
          <p className="subtitle">Review and final approve CIA question papers submitted by HOD</p>
        </div>
        ...
        {!selectedPaper ? (
          <div className="card">
            {loading ? (
              ...
              <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
                No question papers pending principal review.
              </div>
            ) : (
                    ...
                    {papers.map(p => (
                      <tr key={p.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                        ...
                        <td style={{ padding: '12px 10px' }}>
                          <span style={{ padding: '4px 8px', borderRadius: 12, fontSize: 11, fontWeight: 'bold', background: '#fef3c7', color: '#92400e' }}>
                            {p.status}
                          </span>
                        </td>
                        <td style={{ padding: '12px 10px' }}>
          ...
          <div className="card">
            ...
            <div style={{ marginTop: 30, padding: 20, border: '1px solid var(--border-color)', borderRadius: 8, background: 'var(--bg-color)' }}>
              <h3 style={{ marginTop: 0 }}>Principal Approval</h3>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => handleAction('APPROVED')} disabled={actionLoading} style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: '#10b981', color: '#fff', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <CheckCircle size={18} /> Final Approve Paper
                </button>

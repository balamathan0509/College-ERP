...
export default function StaffTimetable() {
  ...
  // ── Fetch regular ────────────────────────────────────────────────────────────
  async function fetchRegularTimetable() {
  ...
  // ── When selectedYear changes → sync all exam rows' year ────────────────────
  useEffect(() => {
  ...
  // ── Save regular ─────────────────────────────────────────────────────────────
  async function saveRegularTimetable() {
    setSaving(true);
    try {
  ...
  function downloadExamExcel() {
    ...
    XLSX.writeFile(wb, `${userProfile.dept}_${examType}_${selectedYear}_Timetable.xlsx`);
  }
  ...
  useEffect(() => { if (selectedYear) fetchRegularTimetable(); }, [selectedYear]);
  useEffect(() => { if (selectedYear) fetchExamTimetable(); }, [examType, selectedYear]);
  ...
  return (
    <div className="dashboard-wrapper">
      ...
      <main className="main-content">
        ...
        {/* ── Shared Year Selector — shown on BOTH tabs ── */}
          ...
          <>
            {!selectedYear ? (
              <div className="card" style={{ textAlign: "center", padding: 60 }}>
                ...
                <p style={{ color: "#a0aec0" }}>Select a Year above to manage timetable</p>
              </div>
            ) : fetching ? (
              ...
              <div className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
                  <h3 style={{ fontFamily: "Syne", fontSize: 18 }}>Regular Timetable — {selectedYear}</h3>
                  <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    <button onClick={saveRegularTimetable} disabled={saving} className="btn-primary" style={{ width: "auto", padding: "10px 24px" }}>
                      {saving ? "Saving..." : "💾 Save"}
                    </button>
        ...
        {tab === "exam" && (
          <>
            ...
            {!selectedYear ? (
              <div className="card" style={{ textAlign: "center", padding: 60 }}>
                <div style={{ fontSize: 48, marginBottom: 12 }}>📝</div>
                <p style={{ color: "#a0aec0" }}>Select a Year above to manage exam timetable</p>
              </div>
            ) : (
              <div className="card">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    <button onClick={saveExamTimetable} disabled={examSaving} className="btn-primary" style={{ width: "auto", padding: "10px 24px" }}>
                    ...
                    <tbody>
                      {examRows.map((row, idx) => (
                        <tr key={idx} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                          ...
                          <td style={{ padding: "6px 8px" }}>
                            <button onClick={() => removeExamRow(idx)} style={{
                ...
                <button onClick={addExamRow} style={{
